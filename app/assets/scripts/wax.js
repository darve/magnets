...
(function (win, doc, c) {
    ...
    points = [],
        bg = 'rgb(0, 5, 25)',
        pal = ['rgba(0, 91, 197,', 'rgba(0, 180, 252,', 'rgba(23, 249, 255,', 'rgba(223, 147, 0,', 'rgba(248, 190, 0,'],


    // global configuration
    vector_scale = 0.01,
        // vector scaling factor, we want small steps
    ...
    function render() {
        ...
        if (delta > interval) {
            ...
            points.map(function (p, i) {
                ...
                var xx = P.map(p.x, -5, 0, 0, w);
                var yy = P.map(p.y, -5, 0, 0, h);
                ...
                var n = Math.PI * 2 * P.noise(p.x, p.y);
                var v = new Vec(Math.cos(n), Math.sin(n));
